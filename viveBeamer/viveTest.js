...
function init() {
    ...
    boxEntity = Entities.addEntity({
        ...
        locked: false,
        lifetime: 60000,
        script:Script.resolvePath('beamer.js')
    });
}
...
function update(dt) {
    ...
    if (thumbDown) {
        var rx = Controller.getValue(Controller.Hardware.Vive.RX);
        var ry = Controller.getValue(Controller.Hardware.Vive.RY);

        var lx = Controller.getValue(Controller.Hardware.Vive.LX);
        var ly = Controller.getValue(Controller.Hardware.Vive.LY);

        var xOffset = Vec3.multiply(boxZAxis, rx);
        var yOffset = Vec3.multiply(boxYAxis, ry);
